...
export const partnerregister = async (req, res, next) => {
  try {
    const salt = bcrypt.genSaltSync(10);
    const hash = bcrypt.hashSync(req.body.password, salt);
...
export const partnerlogin = async (req, res, next) => {
  try {
    ...
    if (!partneruser) return next(createError(404, "User not found"));
    ...
    const token = jwt.sign(
      { id: partneruser._id, isPartner: partneruser.isPartner },
      process.env.JWT
    );
    const { password, isPartner, ...otherDetails } = partneruser._doc;
    res
      .cookie("access_token", token, { httpOnly: true })
      .status(200)
      .json({ details: { ...otherDetails }, isPartner });
  } catch (err) {
    next(err);
  }
};